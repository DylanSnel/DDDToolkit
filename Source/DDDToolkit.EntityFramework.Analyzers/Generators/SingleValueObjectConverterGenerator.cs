...
public class SingleValueObjectConverterGenerator : IIncrementalGenerator
{
    ...
    private static void GenerateConfigurationExtensions(SourceProductionContext sourceContext, ((DDDOptions Properties, ImmutableArray<GenericObjectDefinition> classes) data, Compilation compilation) context)
    {
        var classes = context.data.classes;
        ...
        var distinctNamespaces = classes.Select(c => c.Namespace).Distinct();
        ...
        var propertiesConfigurations = string.Join("\n        ", classes.Select(c => $"modelConfigurationBuilder.Properties<{c.Name}>().HaveConversion<{c.Name}Converter>();"));
        ...
        var sourceCode = $$"""
            ...
            public static class SingleValueObjectConverterExtensions
            {
                public static void Add{{moduleName}}SingleValueObjectConverters(this ModelConfigurationBuilder modelConfigurationBuilder)
                {
        ...
        sourceContext.AddSource("SingleValueObjectConverterExtensions.g.cs", SourceText.From(sourceCode, Encoding.UTF8));
    }
