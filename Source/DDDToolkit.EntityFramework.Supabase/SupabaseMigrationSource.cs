...
/// <para>
/// A module declares one and hands it to both sides. The export needs nothing but the design-time
/// factory <c>dotnet ef</c> already uses, so it runs without building the host: no configuration, no
/// Azure App Configuration, no hosted services. The start-up check needs the application's own
/// context, so it takes it from the container once the host exists.
/// </para>
/// <code>
...
/// }
///
/// // anywhere, no host: a test, a small console app, the top of Program.cs
...
public sealed class SupabaseMigrationSource
{
    ...
    private SupabaseMigrationSource(Type contextType, Func<DbContext> createDesignTime, Func<IServiceProvider, DbContext> resolve)
    {
    ...
    /// </summary>
    public static SupabaseMigrationSource For<TContext, TFactory>()
        where TContext : DbContext
        where TFactory : IDesignTimeDbContextFactory<TContext>, new()
        => new(typeof(TContext), static () => new TFactory().CreateDbContext([]), static services => services.GetRequiredService<TContext>());
    ...
    public static SupabaseMigrationSource For<TContext>(Func<TContext> createDesignTime) where TContext : DbContext
    {
        ArgumentNullException.ThrowIfNull(createDesignTime);
        return new(typeof(TContext), createDesignTime, static services => services.GetRequiredService<TContext>());
    }
